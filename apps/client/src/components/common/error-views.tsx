...
export const ErrorView: React.FC<ErrorViewProps> = ({ status, message, onRetry }) => {
    ...
    switch (status) {
      case 403:
        return {
          ...
          description: message || t("error.403.description", "Vous n'avez pas les permissions nécessaires pour accéder à cette ressource."),
          icon: (
      ...
      case 429:
        return {
          ...
          description: t("error.429.description", "Vous avez envoyé trop de requêtes en peu de temps. Veuillez patienter quelques minutes avant de réessayer."),
          icon: (
          ...
          action: (
            <Button color="danger" variant="flat" onPress={onRetry || (() => window.location.reload())}>
      ...
      case 404:
        return {
          ...
          description: t("error.404.description", "La ressource que vous recherchez n'existe pas ou a été déplacée."),
          icon: (
      ...
      default:
        return {
          ...
          description: t("error.500.description", "Nous rencontrons un problème technique. Nos équipes ont été notifiées."),
          icon: (
            <svg className="w-12 h-12 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          action: (
            <Button color="primary" onPress={onRetry || (() => window.location.reload())}>
  ...
  return (
    <div className="flex items-center justify-center p-4 min-h-[400px]" role="alert">
      <Card className="max-w-md w-full border-none bg-background/60 dark:bg-default-100/50 backdrop-blur-md shadow-xl overflow-hidden">
        <CardBody className="p-8 text-center flex flex-col items-center gap-6">
          <div className="w-20 h-20 rounded-3xl bg-default-100 dark:bg-default-50 flex items-center justify-center shadow-inner" aria-hidden="true">
            {config.icon}
          </div>
