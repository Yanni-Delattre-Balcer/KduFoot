...
import { CreateMatchDto, Match, Level, Venue, PitchType } from '@/types/match.types';
...
export default function MatchForm({ initialData, onSuccess, onCancel }: MatchFormProps) {
    ...
    useEffect(() => {
        if (initialData) {
            ...
            setFormData(prev => ({
                ...
                jersey_color: prev.venue === 'Domicile' ? user.home_jersey_color || '' : (prev.venue === 'Extérieur' ? user.away_jersey_color || '' : '')
            }));
        }
    }, [initialData, user]);
    ...
    const handleChange = (field: keyof CreateMatchDto, value: any) => {
        ...
        if (field === 'phone') {
            ...
            if (user) {
                ...
                else if (venue === 'Extérieur') jerseyColor = user.away_jersey_color || '';
            }
            setFormData(prev => ({ ...prev, [field]: value, jersey_color: jerseyColor }));
        } else {
    ...
    const validate = () => {
        ...
        if (!formData.email || !/^\S+@\S+\.\S+$/.test(formData.email)) newErrors.email = t('account.errors.email_invalid');
        if (!formData.phone || formData.phone.replace(/\D/g, '').length < 11) newErrors.phone = t('account.errors.phone_required');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (initialData?.id) {
                ...
                addToast({ title: t('success'), description: t('matchForm.alerts.update_success'), color: 'success' });
            } else {
                await createMatch(payload as any);
                addToast({ title: t('success'), description: t('matchForm.alerts.create_success'), color: 'success' });
            }
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            >
                                ...
                                <SelectItem key="Extérieur">Nous nous déplaçons (Extérieur)</SelectItem>
                            </Select>
                            <Select
                                label="Type terrain"
                    ...
                    <div className="flex flex-col gap-2 mt-4">
                        ...
                        <Progress 
                            ...
                            classNames={{
                                ...
                                indicator: "bg-gradient-to-r from-violet-600 to-violet-400 rounded-full"
                            }}
                        />
                    </div>
                </CardBody>
