...
export default function TournamentForm({ onSuccess, onCancel }: TournamentFormProps) {
    ...
    const handleLinkClub = async () => {
        ...
        if (!cleanSiret || cleanSiret.length !== 14) {
            addToast({ title: t('error.title'), description: t('matchForm.alerts.siret_length', 'Le numéro SIRET doit contenir 14 chiffres.'), variant: 'flat', color: 'danger' });
            return;
        }
        setIsLinking(true);
        try {
            await linkClub(cleanSiret);
            addToast({ title: t('success', 'Succès'), description: t('matchForm.alerts.link_success', 'Club lié avec succès'), variant: 'flat', color: 'success' });
        } catch (error: any) {
            addToast({ title: t('error.title'), description: error.message || t('matchForm.alerts.link_error', 'Erreur lors de la liaison du club.'), variant: 'flat', color: 'danger' });
        } finally {
            setIsLinking(false);
        }
    };
            ...
            <Card className="shadow-medium border-purple-300/40 bg-purple-300/10">
                ...
                <CardBody className="grid grid-cols-1 md:grid-cols-4 gap-6 p-6">
                    ...
                    <div className="md:col-span-2">
                        {!user?.club_id ? (
                            <div className="p-5 bg-purple-300/20 border-2 border-purple-400/50 rounded-2xl flex flex-col gap-5 shadow-sm h-full">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <h4 className="text-sm md:text-base font-black text-purple-100 uppercase tracking-tighter">
                                            {t('matchForm.link_club.title')}
                                        </h4>
                                        <div className="text-[12px] md:text-[14px] text-purple-200 space-y-2 font-bold leading-snug">
                                            <p className="underline decoration-2 text-purple-100">{t('matchForm.link_club.warning_siret')}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex flex-col sm:flex-row gap-3 items-center w-full justify-start mt-auto">
                                    <Input
                                        ...
                                        classNames={{
                                            inputWrapper: "bg-white border-purple-400/50 shadow-inner h-10 font-bold text-black"
                                        }}
                                    />
                                    <Button size="md" onPress={handleLinkClub} isLoading={isLinking} className="bg-purple-300 text-purple-950 w-full sm:w-auto font-black px-8 shadow-md h-10 uppercase tracking-tighter">
