...
export const DataWall: React.FC<DataWallProps> = ({ message: customMessage, children }) => {
    ...
    // 0. ACCÈS TOTAL : Si l'utilisateur est connecté ET son profil est complet, on rend tout brut et net
    ...
    // 4. MODE VISITEUR (Connecté profil incomplet + choix "Visiter")
    // Le fond reste flou, mais on affiche un overlay plus léger avec le message de déblocage
    if (isVisitor) {
        return (
            <div className="relative w-full h-full min-h-[400px]">
                ...
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center p-4 animate-appearance-in bg-black/5 backdrop-blur-[2px] rounded-3xl">
                    <div className="bg-content1/90 border border-default-200 shadow-xl rounded-3xl p-6 max-w-sm w-full text-center flex flex-col items-center gap-4">
                        <div className="p-3 rounded-full bg-warning-100/50 text-warning border border-warning-200">
                        ...
                        <Button
                            ...
                            className="bg-warning text-white font-black uppercase tracking-tighter w-full rounded-xl h-12 shadow-lg shadow-warning/20"
                            size="md"
                        >
    ...
    // 5. CONNECTÉ MAIS PROFIL INCOMPLET : Grande modale de blocage initiale
    if (isAuthenticated && !profileComplete) {
        return (
            <div className="relative w-full h-full min-h-[400px]">
                <div className="blur-sm pointer-events-none select-none">
                ...
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center p-4 animate-appearance-in bg-black/20 backdrop-blur-[4px] rounded-3xl">
    ...
    // This return should ideally not be reached if all conditions are covered,
    // but as a fallback or if children should be rendered by default.
    return <>{children}</>;
};
