...
export default function MatchesPage() {
    ...
    const { user, isLoading: isUserLoading, isLocked } = useUser();
        ...
        <DefaultLayout maxWidth="max-w-full">
            <section className="flex flex-col gap-6 w-full px-4">
                ...
                <div className={`relative overflow-hidden rounded-3xl ${uiConfig.gradient} border ${uiConfig.border}`}>
                    {/* Grass stripes */}
                    <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'repeating-linear-gradient(90deg, transparent, transparent 40px, rgba(34,197,94,0.3) 40px, rgba(34,197,94,0.3) 80px)' }}></div>
                    {/* Field center line + circle */}
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-full bg-linear-to-b from-transparent via-white/5 to-transparent pointer-events-none"></div>
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full border border-white/5 pointer-events-none"></div>
                        ...
                        <div className="flex flex-col gap-4 items-center">
                            ...
                            <div className="relative z-20 flex flex-col sm:flex-row justify-center gap-3 p-1.5 rounded-2xl bg-default-100/50 backdrop-blur-md w-full sm:w-auto border border-white/5">
                                ...
                                <Button
                                    ...
                                    onPress={() => {
                                        if (isVisitor) {
                                            openGateway("Veuillez vous connecter pour créer une annonce.");
                                            return;
                                        }
                                        setView('create');
                                    }}
                            ...
                            <div className="flex flex-col gap-5">
                                {(isVisitor || isLocked) && (
                                    <DataWall
                                        ...
                                        isVisitor={isVisitor}
                                        onLogin={() => openGateway()}
                                        message={isLocked ? "L'accès aux recherches détaillées est réservé aux profils complets." : undefined}
                                    />
                                )}
