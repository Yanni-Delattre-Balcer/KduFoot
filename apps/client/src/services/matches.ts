...
export const matchService = {
    ...
    create: async (data: CreateMatchDto, token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}`, {
            method: 'POST',
            headers: {
    ...
    update: async (id: string, data: UpdateMatchDto, token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}/${id}`, {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
    ...
    contact: async (id: string, data: ContactMatchDto, token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}/${id}/contact`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
    ...
    getRequests: async (token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}/requests`, {
            method: 'GET',
            headers: { Authorization: `Bearer ${token}` },
        });
        if (!response.ok) {
    ...
    getParticipations: async (token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}/participations`, {
            method: 'GET',
            headers: { Authorization: `Bearer ${token}` },
        });
        if (!response.ok) {
    ...
    updateRequestStatus: async (matchId: string, userId: string, status: 'accepted' | 'refused', token: string) => {
        const response = await fetch(`${import.meta.env.BASE_API_URL}${BASE_URL}/${matchId}/requests/${userId}`, {
            method: 'PATCH',
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
